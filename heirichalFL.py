...
import numpy as np
import copy 
isGroupGradientsToBeAveraged = True 
bypass_robust= False # normalizes groups gradients and sums them
simple_average= False # simple average of group gradients
averageOverGroupLen= True # average over group length, else average over number of users
skip_filtering = False # skip filtering of malicious users
safeFL_approach = False # robust aggregation mechanism
median_aggregation = False
bestGroup = False #use the best group for aggregation
...
def filter_malicious_users(groups_with_users, all_user_trust_scores, malicious_percentage, total_user_count):
    ...
    filtered_users = [user[0] for user in users_sorted_by_score[:users_to_exclude_count]]
    malicious_user_ids = set(filtered_users)
    ...
    for group_id, group_users in enumerate(groups_with_users):
        ...
        if len(trusted_user_ids) > 1:
            ...
            filtered_users.extend(trusted_user_ids)
    

    # make sure all filtered groups have more than 1 user
    assert all(len(group[1]) > 1 for group in filtered_groups_with_ids), "210: All groups must have more than 1 user"

    return filtered_groups_with_ids, filtered_users
...
def compute_group_gradients(filtered_groups_with_ids, user_gradients, gradient_shape, computation_device):
    ...
    for group_id, trusted_user_ids in filtered_groups_with_ids:
        if not trusted_user_ids:  # Skip if no users in group (shouldn't happen)
            assert False, "Group must have at least one user"
...
def aggregate_groups(user_gradient_vectors, computation_device, random_seed, hierarchical_parameters, skip_filtering=False):
    ...
    if skip_filtering:
        ...
        for group_id, group_users in enumerate(groups_with_users):
            ...
            if len(user_ids) > 1:
                ...
                filtered_users.extend(user_ids)
        ...
        filtered_groups_with_ids, filtered_users = filter_malicious_users(groups_with_users, user_trust_scores, malicious_percentage, total_user_count)
    ...
    if len(filtered_groups_with_ids) == 0:
        ...
        assert False, "397: All groups were filtered out"
        filtered_groups_with_ids = handle_filtering_fallback(groups_with_users, user_trust_scores)
    

    # make sure that all groups in the filtered groups have more than 1 user
    assert all(len(group[1]) > 1 for group in filtered_groups_with_ids), "402: All groups must have more than 1 user"
    ...
    return group_to_gradient_mapping, filtered_users
...
def score_groups(group_to_gradient_mapping, hierarchical_parameters):
    ...
    if number_of_existing_groups < 3:
        return _create_default_scores_(existing_group_ids)
    
    # Calculate similarity matrix
    cos_sim = _calculate_similarity_matrix_(group_to_gradient_mapping, existing_group_ids)
    # Perform ensemble voting using similarity matrix
    group_votes = _perform_ensemble_voting_(cos_sim, number_of_existing_groups)
    
    # Convert votes to dictionary with original group IDs
    group_id_to_score = _convert_votes_to_scores_(group_votes, existing_group_ids)


    #calculate norm scores
    norms = _calculate_gradient_distance_matrix_(group_to_gradient_mapping, existing_group_ids)

    group_votes_norm = _perform_ensemble_voting_(norms, number_of_existing_groups)
   
    norm_group_id_to_score = _convert_votes_to_scores_(group_votes_norm, existing_group_ids)

    

    # Combine scores using adaptive threshold
    combined_scores = {}
    for group_id in existing_group_ids:
        combined_scores[group_id] = group_id_to_score[group_id] - norm_group_id_to_score[group_id]
...
def update_user_scores(heirichal_params, groups_scores):
    ...
    minimum_score = min(group_adjustments.values())
    #give the minimum score to all groups that were filtered out
    for group_id in range(number_of_groups):
        if group_id not in group_adjustments:
            group_adjustments[group_id] = minimum_score
...
def robust_groups_aggregation(group_gradients, net, lr, device, heirichal_params, number_of_users):
    ...
    # Use robust aggregation
        ...
        for group_id, grad in group_gradients.items():
            ...
            elif group_norms[group_id] > median_norm:
                 
                scaling_factor = median_norm / group_norms[group_id]
                filtered_gradients[group_id] = grad * (scaling_factor ** 3)
    ...
    elif median_aggregation:

        #Take the median of the gradients
        gradients = torch.stack(list(group_gradients.values()), dim=0)
        global_update = torch.median(gradients, dim=0).values

    elif bestGroup:
        #requires the parameter group_scores
        #find the best group and use its gradient
        group_scores = heirichal_params["current group scores"]
        
        # sort groups by their scores
        sorted_groups = sorted(group_scores.items(), key=lambda x: x[1], reverse=True)
        #get the best group id that corresponds to one of the group_gradients keys
        for group_id, _ in sorted_groups:
            if group_id in group_gradients:
                best_group_id = group_id
                break


        global_update = group_gradients[best_group_id]
...
def _create_default_scores_(group_ids):
    """
    Create default scores (1.0) for all groups when there are too few groups to score.
    
    Args:
        group_ids: List of group IDs
        
    Returns:
        Dictionary mapping group IDs to default scores (1.0)
    """
    return {group_id: 1.0 for group_id in group_ids}


def _calculate_similarity_matrix_(group_to_gradient_mapping, existing_group_ids):
    """
    Calculate pairwise cosine similarities between all existing groups.
    
    Args:
        group_to_gradient_mapping: Dictionary mapping group IDs to their aggregated gradients
        existing_group_ids: List of existing group IDs
        
    Returns:
        Tensor containing pairwise cosine similarities
    """
    number_of_existing_groups = len(existing_group_ids)
    ...
    for i in range(number_of_existing_groups):
        ...
        for j in range(i+1, number_of_existing_groups):
            ...
            similarity = _calculate_cosine_similarity_(
...
def _calculate_cosine_similarity_(gradient_1, gradient_2):
    """
    Calculate cosine similarity between two gradients.
    
    Args:
        gradient_1: First gradient tensor
        gradient_2: Second gradient tensor
        
    Returns:
        Cosine similarity between the gradients
    """
    return F.cosine_similarity(gradient_1, gradient_2, dim=0, eps=1e-9)


def _perform_ensemble_voting_(similarity_matrix, number_of_groups):
    """
    Perform ensemble voting where each group votes on other groups' trustworthiness.
    
    Args:
        similarity_matrix: Matrix of pairwise cosine similarities
        number_of_groups: Number of groups participating in voting
        
    Returns:
        Tensor containing accumulated votes for each group
    """
    group_votes = torch.zeros(number_of_groups)
...
def _convert_votes_to_scores_(group_votes, existing_group_ids):
    """
    Convert votes tensor to dictionary mapping original group IDs to scores.
    
    Args:
        group_votes: Tensor containing accumulated votes for each group
        existing_group_ids: List of existing group IDs
    ...
    return {
        ...
        for i in range(len(existing_group_ids))
    }

def _calculate_gradient_distance_matrix_(group_to_gradient_mapping, existing_group_ids):
    """
    Calculate a distance matrix between each pair of gradients to measure their dissimilarity.
    ...
    Returns:
        Tensor containing the distance matrix between all pairs of gradients
    """
    number_of_existing_groups = len(existing_group_ids)
    distance_matrix = torch.zeros((number_of_existing_groups, number_of_existing_groups))
    
    # Calculate distance between each pair of gradients
    for i, group_id_i in enumerate(existing_group_ids):
        gradient_i = group_to_gradient_mapping[group_id_i]
        
        for j, group_id_j in enumerate(existing_group_ids):
            # If comparing the same gradient, distance is 0
            if i == j:
                distance_matrix[i, j] = 0
                continue
                
            gradient_j = group_to_gradient_mapping[group_id_j]
            
            # Calculate Euclidean distance between the two gradients
            distance = torch.norm(gradient_i - gradient_j).item()
            distance_matrix[i, j] = distance
    
    # Normalize the distance matrix if there are non-zero values
